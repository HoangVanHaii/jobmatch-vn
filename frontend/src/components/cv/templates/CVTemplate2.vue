...
<template>
  <div class="w-full min-h-[1100px] bg-white text-neutral-700 font-sans text-[13px] leading-relaxed flex flex-col">
    <!-- Header -->
    <header class="relative overflow-hidden bg-[#a8b3c0] px-10 pt-9 pb-7 flex items-center">
      <!-- Shape nền trang trí -->
      ...
      <div class="relative z-[1] flex items-center gap-6 w-full">
        <!-- Avatar -->
        <div class="w-[130px] h-[130px] rounded-full bg-white p-1 shrink-0">
          <div class="w-full h-full rounded-full overflow-hidden bg-neutral-300 flex items-center justify-center">
            ...
            />
            <span v-else class="text-[44px] font-semibold text-stone-600">
        ...
        <div class="flex-1">
          <h1 class="text-[30px] font-bold text-neutral-800 m-0">
            {{ data.personalInfo.fullName || 'Họ và tên' }}
          </h1>
          <p class="text-[16px] text-amber-700 font-semibold mt-1">
            {{ data.personalInfo.position || 'Vị trí ứng tuyển' }}
          </p>
          <p v-if="data.summary" class="text-[12.5px] mt-3 text-neutral-600 max-w-[520px]">
            {{ data.summary }}
          </p>
        </div>
      </div>
    ...
    <!-- Body 2 cột -->
    <div class="grid grid-cols-2 gap-8 px-10 pt-7 pb-9 flex-1">
      <!-- Cột trái -->
      <section class="flex flex-col gap-4">
        <h2 class="text-base font-bold text-amber-700 m-0 pb-1.5 border-b-[1.5px] border-amber-700">
          Thông tin cá nhân
        </h2>
        <ul class="list-none p-0 m-0 flex flex-col gap-1.5">
          ...
          <li v-if="data.personalInfo.facebook" class="flex items-center gap-2 text-[12.5px]">
        ...
        <template v-if="data.educations.length">
          ...
          <div v-for="(e, i) in data.educations" :key="i" class="mb-2">
            <p class="text-[13px] mb-1 text-neutral-800">
              <strong>{{ e.school }}</strong> | {{ e.startYear || '' }} - {{ e.endYear || 'Nay' }}
            </p>
            <p class="text-[12.5px] my-0.5 text-neutral-600"><strong>Chuyên ngành:</strong> {{ e.major || '—' }}</p>
            <p v-if="e.degree" class="text-[12.5px] my-0.5 text-neutral-600"><strong>Xếp loại:</strong> {{ e.degree }}</p>
            <p v-if="e.description" class="text-[12.5px] my-0.5 text-neutral-600">
        ...
        <template v-if="data.skills.length">
          ...
          <ul class="list-none p-0 m-0 flex flex-col gap-1.5">
            <li
              ...
              :key="i"
              class="relative pl-4 text-[12.5px]"
            >
        ...
        <template v-if="data.certificates.length">
          ...
          <div v-for="(c, i) in data.certificates" :key="i" class="mb-2">
            <p class="text-[13px] mb-1 text-neutral-800"><strong>{{ c.date || '' }}</strong></p>
            <p class="text-[12.5px] my-0.5 text-neutral-600">
      ...
      <section class="flex flex-col gap-4">
        <template v-if="data.experiences.length">
          ...
          <div v-for="(x, i) in data.experiences" :key="i" class="mb-2">
            ...
            <p class="text-[12.5px] my-0.5 text-neutral-600"><strong>{{ x.position }}</strong></p>
            <p v-if="x.description" class="text-[12.5px] mt-1 whitespace-pre-wrap">{{ x.description }}</p>
          </div>
        </template>
        ...
        <template v-if="data.activities.length">
          ...
          <div v-for="(a, i) in data.activities" :key="i" class="mb-2">
            ...
            <p v-if="a.role" class="text-[12.5px] my-0.5 text-neutral-600">{{ a.role }}</p>
            <p v-if="a.description" class="text-[12.5px] mt-1 whitespace-pre-wrap">{{ a.description }}</p>
          </div>
        </template>
      </section>
    </div>
