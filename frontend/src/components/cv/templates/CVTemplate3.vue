...
<template>
  <div class="w-full min-h-[1100px] bg-white text-neutral-800 font-serif text-[13px] leading-relaxed px-10 pt-9 pb-10 flex flex-col gap-[18px]">
    <!-- Header centered -->
    <header class="text-center mb-2">
      <h1 class="text-[26px] font-bold m-0 tracking-[0.3px]">
        {{ data.personalInfo.fullName || 'Họ và tên' }}
      </h1>
      <p class="text-[14px] italic mt-1 text-neutral-600">
        {{ data.personalInfo.position || 'Vị trí ứng tuyển' }}
      </p>
      <p class="flex flex-wrap justify-center gap-3.5 text-[12px] mt-2.5 text-neutral-500">
    ...
    <section v-if="data.summary" class="w-full">
      <h2 class="font-sans text-[13px] font-bold tracking-[1px] mb-1 m-0">MỤC TIÊU NGHỀ NGHIỆP</h2>
      <hr class="border-0 border-t-[1.5px] border-neutral-800 mb-2.5" />
      <p class="text-[12.5px] mt-1.5 whitespace-pre-wrap">{{ data.summary }}</p>
    </section>

    <section v-if="data.educations.length" class="w-full">
      <h2 class="font-sans text-[13px] font-bold tracking-[1px] mb-1 m-0">HỌC VẤN</h2>
      <hr class="border-0 border-t-[1.5px] border-neutral-800 mb-2.5" />
      <div v-for="(e, i) in data.educations" :key="i" class="mb-3">
        <p class="flex justify-between items-baseline m-0">
          <strong>{{ e.school }}</strong>
          <span v-if="e.startYear || e.endYear" class="text-[12px] text-neutral-500">
            {{ e.startYear || '' }} - {{ e.endYear || 'Nay' }}
          </span>
        </p>
        <p class="text-[12.5px] mt-0.5 mb-0">
          <em>{{ e.major || '' }}</em>{{ e.degree ? ' · ' + e.degree : '' }}
        </p>
        <p v-if="e.description" class="text-[12.5px] mt-1.5 whitespace-pre-wrap">{{ e.description }}</p>
      </div>
    </section>

    <section v-if="data.experiences.length" class="w-full">
      <h2 class="font-sans text-[13px] font-bold tracking-[1px] mb-1 m-0">KINH NGHIỆM LÀM VIỆC</h2>
      <hr class="border-0 border-t-[1.5px] border-neutral-800 mb-2.5" />
      <div v-for="(x, i) in data.experiences" :key="i" class="mb-3">
        <p class="flex justify-between items-baseline m-0">
          <strong>{{ x.company }}</strong>
          <span v-if="x.startDate || x.endDate" class="text-[12px] text-neutral-500">
            {{ x.startDate || '' }} - {{ x.endDate || 'Nay' }}
          </span>
        </p>
        <p class="text-[12.5px] mt-0.5 mb-0"><em>{{ x.position }}</em></p>
        <p v-if="x.description" class="text-[12.5px] mt-1.5 whitespace-pre-wrap">{{ x.description }}</p>
      </div>
    </section>

    <section v-if="data.skills.length" class="w-full">
      <h2 class="font-sans text-[13px] font-bold tracking-[1px] mb-1 m-0">KỸ NĂNG</h2>
      <hr class="border-0 border-t-[1.5px] border-neutral-800 mb-2.5" />
    ...
    <section v-if="data.activities.length" class="w-full">
      <h2 class="font-sans text-[13px] font-bold tracking-[1px] mb-1 m-0">HOẠT ĐỘNG</h2>
      <hr class="border-0 border-t-[1.5px] border-neutral-800 mb-2.5" />
      <div v-for="(a, i) in data.activities" :key="i" class="mb-3">
        <p class="flex justify-between items-baseline m-0">
          <strong>{{ a.name }}</strong>
          <span v-if="a.time" class="text-[12px] text-neutral-500">{{ a.time }}</span>
        </p>
        <p v-if="a.role" class="text-[12.5px] mt-0.5 mb-0"><em>{{ a.role }}</em></p>
        <p v-if="a.description" class="text-[12.5px] mt-1.5 whitespace-pre-wrap">{{ a.description }}</p>
      </div>
    </section>
  </div>
</template>